...
AS $$
  ...
  -- "Completed Core Courses" = users who finished ALL 8 first_steps tasks
  -- (Connect to Discord = 1 task + Onboarding Steps = 7 tasks)
  core_completed AS (
    SELECT count(*) AS total FROM phase_counts
    WHERE phase = 'first_steps' AND cnt >= 8
  ),
  badges_all AS (
    SELECT count(*) AS total FROM phase_counts
    WHERE (phase = 'first_steps' AND cnt >= 8)
  ...
  -- Rolling window: users who crossed the 8-task threshold this week
  rw_core_completed AS (
    SELECT count(*) AS total FROM (
      SELECT pc.user_id FROM phase_counts pc
      WHERE pc.phase = 'first_steps' AND pc.cnt >= 8
        AND COALESCE((SELECT ppc.cnt FROM pre_rw_counts ppc WHERE ppc.user_id = pc.user_id AND ppc.phase = 'first_steps'), 0) < 8
