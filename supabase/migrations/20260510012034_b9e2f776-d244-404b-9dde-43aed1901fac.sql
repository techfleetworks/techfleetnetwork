...
project_counts AS (
  SELECT
    -- Open applications = projects actively accepting applications.
    count(*) FILTER (WHERE project_status = 'apply_now') AS open_applications,
    -- Coming soon = projects in the pre-launch pipeline (announced, recruiting,
    -- or onboarding the team) but not yet running and not yet open for apps.
