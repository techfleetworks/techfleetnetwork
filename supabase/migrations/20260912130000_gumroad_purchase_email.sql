...
-- ADR-0042. EXPAND-only, idempotent. Does not touch the ledger/projector (ADR-0037)
...
COMMENT ON FUNCTION public.trg_gumroad_sales_purchase_email() IS
  'UC2 (ADR-0042): enqueues a Tier-0 founding-purchase email when a membership sale is linked to a user. Idempotent via email_outbox idempotency_key = founding-purchase:<sale_id>. Membership-scoped, active-only, never rolls back the sale.';
