...
VALUES (
  ...
  61,
  'Dashboard shows submitted general application status',
  'Feature: Dashboard Application Status
  Scenario: Submitted general application is prioritized over stale draft state
    Given a member has a completed general application
    And the member opens the dashboard
    When the dashboard loads the My Applications section
    Then the general application card shows Submitted
    And it does not show Draft for that completed application',
  'implemented',
  'unit',
  'src/pages/DashboardPage.tsx',
  'Regression coverage for dashboard status sync with the Applications page.'
)
