...
-- SECURITY DEFINER function: returns only aggregate counts, no PII
...
AS $$
  SELECT json_build_object(
    ...
    'third_steps_completed', (
      ...
      JOIN (
        SELECT jp2.user_id
        FROM public.journey_progress jp2
        WHERE jp2.phase = 'third_steps' AND jp2.completed = true
        GROUP BY jp2.user_id
        HAVING count(*) >= 1
      ) sub ON jp.user_id = sub.user_id
      WHERE jp.phase = 'third_steps'
    )
  );
$$;
