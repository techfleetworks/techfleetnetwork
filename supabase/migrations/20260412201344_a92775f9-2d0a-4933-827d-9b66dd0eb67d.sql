...
WITH
  ...
  -- Pre-previous-week counts (for detecting new completions in the reporting week)
  pre_pw_agg AS (
    SELECT user_id, phase, count(*) AS cnt
    FROM public.journey_progress
    WHERE completed = true
  ...
  apps_completed AS (
    SELECT count(*) AS total FROM public.general_applications WHERE status = 'submitted'
  ),
