...
-- Baseline table for Airtable-sourced network stats inputs
CREATE TABLE IF NOT EXISTS public.network_stats_baselines (
  id smallint PRIMARY KEY DEFAULT 1,
  airtable_general_apps integer NOT NULL DEFAULT 890,
  airtable_service_leadership_unique integer NOT NULL DEFAULT 1101,
  airtable_masterclass_total integer NOT NULL DEFAULT 1881,
...
INSERT INTO public.network_stats_baselines (id) VALUES (1)
ON CONFLICT (id) DO NOTHING;

ALTER TABLE public.network_stats_baselines ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Admins can view baselines" ON public.network_stats_baselines;
CREATE POLICY "Admins can view baselines"
  ON public.network_stats_baselines FOR SELECT
  TO authenticated
  USING (public.has_role(auth.uid(), 'admin'));

-- Service-role only writes (no policy needed; service role bypasses RLS).

-- Rewrite get_network_stats() to read baselines as equations (no magic numbers).
...
WITH baselines AS (
  SELECT
    COALESCE(airtable_general_apps, 890) AS at_apps,
    COALESCE(airtable_service_leadership_unique, 1101) AS at_servlead,
    COALESCE(airtable_masterclass_total, 1881) AS at_mc_total
  FROM public.network_stats_baselines WHERE id = 1
),
rolling_window AS (
  SELECT
...
SELECT json_build_object(
  ...
  -- Beginner courses = unique Service/Servant Leadership Masterclass registrants (Airtable baseline)
  'beginner_courses_active', (SELECT at_servlead FROM baselines),
  -- Advanced courses = total masterclass registrations - service/servant leadership unique
  'advanced_courses_active', GREATEST((SELECT at_mc_total FROM baselines) - (SELECT at_servlead FROM baselines), 0),
  -- General applications = platform-submitted apps + Airtable baseline (equation, always grows)
  'applications_completed', (SELECT total FROM platform_apps) + (SELECT at_apps FROM baselines),
  'badges_earned', (SELECT total FROM core_completed) + (SELECT total FROM platform_apps) + (SELECT at_apps FROM baselines),
  ...
  'baselines_last_synced_at', (SELECT last_synced_at FROM public.network_stats_baselines WHERE id = 1)
);
$function$;

-- updated_at trigger
CREATE OR REPLACE FUNCTION public.set_network_stats_baselines_updated_at()
RETURNS trigger LANGUAGE plpgsql SET search_path = public AS $$
BEGIN NEW.updated_at = now(); RETURN NEW; END $$;

DROP TRIGGER IF EXISTS trg_network_stats_baselines_updated_at ON public.network_stats_baselines;
CREATE TRIGGER trg_network_stats_baselines_updated_at
  BEFORE UPDATE ON public.network_stats_baselines
  FOR EACH ROW EXECUTE FUNCTION public.set_network_stats_baselines_updated_at();
