...
-- Per-phase aggregates: total completed tasks per user per phase, plus the
-- timestamp of the most recent completion (used for rolling-window deltas).
phase_agg AS (
  SELECT
    user_id,
    phase,
...
-- Same aggregate but limited to completions BEFORE the rolling window starts.
-- Lets us detect transitions ("not complete before -> complete now").
phase_agg_pre AS (
  SELECT
    user_id,
    phase,
...
-- Required task counts per phase, mirroring the Courses UI:
--   Onboard tab          -> first_steps      = 7 tasks
--   Core Courses tab:
--     Build an Agile Mindset             -> second_steps      = 25
--     Join Project Training Teams        -> project_training  = 14
--     Join Volunteer Teams               -> volunteer         =  6
--     Discord Learning Series            -> discord_learning  = 19
--     Observer Course                    -> observer          =  7
--     Agile Cross-Functional Team Dyn.   -> third_steps       = 12
...
-- A "course completion" = (user, phase) row where cnt >= required tasks.
...
-- Core metric = SUM of completed core courses across the population.
-- Includes the Onboard tab AND every Core Courses tab entry.
core_completed AS (
...
-- A course counts as "newly completed in the last 7 days" when:
--   - cnt is now >= required, AND
--   - cnt was < required (or zero) before the rolling window started, AND
--   - the most recent completion timestamp falls inside the window.
...
SELECT json_build_object(
  ...
  -- Core = sum of fully-completed core courses (Onboard tab + all 6 Core Courses tab entries).
