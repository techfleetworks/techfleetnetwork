...
- **`http.ts` CORS includes the `x-trace-id`/`x-request-id` preflight headers.** The frontend `invokeEdge` wrapper attaches `x-trace-id` to every call, so a hand-rolled allow-list that omits it makes the browser fail the CORS preflight — the POST never reaches the function and surfaces as `FunctionsFetchError` ("Failed to send a request to the Edge Function") with **zero edge logs**. That's why inline CORS is banned. Enforced by `scripts/ci/check-edge-cors-trace.mjs` (every `invokeEdge` target must source CORS from `_shared/http.ts`, or inline a list containing `x-trace-id`).

  ```ts
  // ❌ never — a hand-rolled allow-list drifts behind the headers the client sends
  const corsHeaders = { "Access-Control-Allow-Headers": "authorization, apikey, content-type" };
  // ✅ always — compose the one owner (already lists x-trace-id / x-request-id)
