...
serve(
  withAuditWrapper("grant-observer-role", async (req) => {
    ...
    if (progErr) {
      await audit(
        admin,
        user.id,
        "observer_role_grant_failed",
        `progress query: ${progErr.message}`
      );
    ...
    if (missing.length > 0) {
      await audit(
        ...
        user.id,
        "observer_role_grant_failed",
        `incomplete lessons: ${missing.join(",")}`
      );
    ...
    if (existing?.projects_role_granted_at && existing?.observers_role_granted_at) {
      return json(
        {
          ok: true,
          alreadyGranted: true,
