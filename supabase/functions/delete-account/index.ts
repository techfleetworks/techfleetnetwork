 ...
 */
Deno.serve(
  withAuditWrapper("delete-account", async (req) => {
    ...
    try {
      ...
      if (!clientKey) {
        throw new Error(
          "Missing SUPABASE_ANON_KEY / SUPABASE_PUBLISHABLE_KEY for authenticated client creation."
        );
      }

      const supabaseUser = createClient(Deno.env.get("SUPABASE_URL")!, clientKey, {
      ...
      if (deleteError) {
        log.error(
          "delete",
          `Failed to delete auth user ${userId} [${requestId}]: ${deleteError.message}`,
          { requestId, userId },
          deleteError
        );
      ...
      log.info("handler", `Account deletion completed for ${userId} [${requestId}]`, {
        requestId,
        userId,
      });
