...
/**
 * Structural backstop for the capability-denial confabulation (the Figma "I can't read links"
 * incident, ADR-0034): a PURE, tested detector. The strict material path (index.ts) runs it on the
 * finished answer and, because a turn that had readable material in front of it cannot honestly
 * claim it can't read that material, REPLACES a matching answer with an honest fallback before any
 * of it reaches the member.
 *
 * Because it now *blocks* (not just logs), precision matters — a false positive would destroy a
 * correct, member-specific review. So it is deliberately narrow on two axes:
 *  - FIRST PERSON only. It flags Fleety denying about ITSELF ("I can't read your link", "I'm
 *    text-only"). A grounded review that quotes the member in the third/second person
 *    ("…stakeholders can't access the board…", "you can't view it without an invite") is NOT a
 *    self-denial and must never be flagged.
 *  - UNAMBIGUOUS access verbs only (open/read/access/peek/browse/fetch). "see"/"view" are excluded
 *    because a review legitimately says "I can't see a clear problem statement on the board".
 * It also excludes the honest, scoped "I couldn't open THAT one link — set it to viewable and
 * resend" message (a single shared link genuinely wasn't readable this turn).
 */
export function detectsCapabilityDenial(text: string): boolean {
  if (typeof text !== "string" || !text) return false;

  // First-person "I'm text-only" style self-claims — always false for Fleety (it reads material).
  if (
    /\b(?:i(?:'|’)?m|i am)\s+(?:just\s+|only\s+)?(?:a\s+)?text[- ]only\b/i.test(text) ||
    /\bi\s+(?:can\s+)?only\s+(?:generate|produce|output)\s+(?:conversational\s+)?text\b/i.test(
      text
    ) ||
    /\bi\s+have\s+no\s+(?:ability|way)\s+to\s+(?:open|read|access)\b/i.test(text)
  ) {
    return true;
  }

  // First-person "I (adv)? can't/couldn't/am unable to … open|read|access|peek|browse|fetch …
  // link|figma|board|external|website" — Fleety refusing to read, about itself.
  const firstPersonDenial =
    /\bi\s+(?:really\s+|still\s+|actually\s+|just\s+|honestly\s+)?(?:can(?:'|’)?t|cannot|can\s?not|couldn(?:'|’)?t|could\s?not|am\s+unable\s+to|am\s+not\s+able\s+to|don(?:'|’)?t\s+have\s+the\s+ability\s+to)\b[^.!?\n]{0,50}\b(?:open|read|access|peek(?:\s+into)?|browse|fetch)\b[^.!?\n]{0,50}\b(?:link|links|url|urls|figma|figjam|board|boards|external|website|web\s?page)\b/i;
  if (firstPersonDenial.test(text)) {
    // Allow the honest, scoped "I couldn't open THAT specific link — make it viewable and resend".
    const scopedToOneLink =
      /\b(?:that|this|the)\s+(?:one|link|board|file|document|doc)\b/i.test(text) &&
      /\b(?:anyone with the link|can view|set it to|resend|share it|viewable)\b/i.test(text);
    return !scopedToOneLink;
  }
