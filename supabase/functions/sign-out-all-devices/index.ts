...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (auth instanceof Response) {
      log.warn("auth", `Missing Authorization header [${requestId}]`, {
        requestId,
      });
      return auth;
    }
    ...
    log.info(
      "revoke",
      `Revoking all sessions for user ${auth.userId} [${requestId}]`,
      { requestId, userId: auth.userId },
    );

    const { error: signOutError } = await getAdminClient().auth.admin.signOut(
      auth.userId,
      "global",
    );
    if (signOutError) {
      log.error(
        "revoke",
        `Failed to revoke sessions for user ${auth.userId} [${requestId}]: ${signOutError.message}`,
        { requestId, userId: auth.userId },
        signOutError,
      );
    ...
    log.info(
      "revoke",
      `All sessions revoked for user ${auth.userId} [${requestId}]`,
      { requestId, userId: auth.userId },
    );
