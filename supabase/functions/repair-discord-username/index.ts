...
serve(
  withAuditWrapper("repair-discord-username", async (req) => {
    ...
    const isLegacyLeadingDot =
      currentIsUsable && typeof currentUsername === "string" && currentUsername.startsWith(".");
    ...
    if (!memberRes.ok) {
      ...
      return new Response(
        JSON.stringify({ repaired: false, reason: "discord-not-member", status: memberRes.status }),
        {
          status: 200,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        }
    ...
    if (!isUsable(liveUsername)) {
      log.warn("repair", `Discord returned unusable username [${requestId}]`, {
        requestId,
        userId,
      });
    ...
    if (updateErr) {
      log.error(
        "repair",
        `Failed to persist repaired username [${requestId}]`,
        { requestId, userId },
        updateErr
      );
    ...
    log.info("repair", `Repaired Discord username for ${userId} [${requestId}]`, {
      requestId,
      userId,
    });
