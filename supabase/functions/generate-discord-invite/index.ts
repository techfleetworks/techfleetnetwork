...
Deno.serve(
  withAuditWrapper("generate-discord-invite", async (req) => {
    ...
    try {
      ...
      for (const channel of candidates) {
        ...
        if (retries > 0) {
          logger.info(
            "create_invite",
            `Invite creation for #${channel.name} succeeded after ${retries} retries`,
            {
        ...
        if (inviteRes.ok) {
          ...
          if (!inviteCode) {
            inviteErrors.push(
              `${channel.name} [${inviteRes.status}]: Missing invite code in Discord response`
            );
            logger.warn(
              "create_invite",
              `Invite creation returned no code for channel ${channel.name}`,
              {
                channelId: channel.id,
      ...
      await writeDiscordAuditLog({
        ...
        userId: user.id,
        tableName: "profiles",
        changedFields: ["invite_type:onboarding", `channel_count:${candidates.length}`],
      });
