...
Deno.serve(
  withAuditWrapper("public-project-openings", async (req) => {
    ...
    try {
      ...
      const { data: projects, error: projectError } = await supabase
        .from("projects")
        .select(
          "id, client_id, project_type, phase, project_status, team_hats, current_phase_milestones, friendly_name, description, is_shipathon, created_at"
        )
      ...
      const { data: clients, error: clientError } = clientIds.length
        ? await supabase
            .from("clients")
            .select("id, name, logo_url, kind:kind::text")
            .in("id", clientIds)
            .eq("status", "active")
        : {
            data: [] as Array<{
      ...
      const applicationSummary = new Map<
        string,
        { project_id: string; total: number; hatCounts: Record<string, number> }
      >();
      for (const application of applicationStats ?? []) {
        const projectId = application.project_id as string;
        const entry = applicationSummary.get(projectId) ?? {
      ...
      const stats = {
        projects_open_applications: projectRows.filter((p) => p.project_status === "apply_now")
          .length,
        // Coming soon = pre-launch pipeline (announced, recruiting team, onboarding team).
        projects_coming_soon: projectRows.filter((p) =>
          ["coming_soon", "recruiting", "team_onboarding"].includes(p.project_status as string)
        ).length,
        ...
        scrubJson(
          {
            ...
            clients: clients ?? [],
            applicationStats: [...applicationSummary.values()],
            stats,
          },
          {
