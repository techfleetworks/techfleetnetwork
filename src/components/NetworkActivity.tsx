        ...
        {showActivity && (
          <>
            ...
            <div className="grid grid-cols-1 min-[560px]:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8 justify-items-start mb-10">
              ...
              <StatCard icon={<FileCheck className="h-5 w-5 text-success" aria-hidden="true" />} value={(safeStats.historical?.general_applications_pre_platform ?? 0) + (safeStats.applications_completed ?? 0)} label="General applications submitted" sublabel={`+${safeStats.applications_completed ?? 0} since platform launch`} colorClass="bg-success/10" />
