...
const clientSchema = z.object({
  name: z.string().trim().min(1, "Client name is required").max(200, "Max 200 characters"),
  website: z.string().trim().min(1, "Website is required").max(500, "Max 500 characters").url("Must be a valid URL"),
  mission: z.string().trim().min(1, "Mission is required").max(2000, "Max 2000 characters"),
  project_summary: z.string().trim().min(1, "Project summary is required").max(5000, "Max 5000 characters"),
  status: z.enum(["active", "inactive"]),
  primary_contact: z.string().trim().min(1, "Primary contact is required").max(200, "Max 200 characters"),
});
  ...
  const handleSubmit = useCallback(() => {
    ...
    if (!result.success) {
      ...
      result.error.issues.forEach((issue) => {
        const key = issue.path[0] as keyof ClientForm;
    ...
    setErrors({});
    if (editingClient) {
      updateMutation.mutate({ id: editingClient.id, values: result.data });
    } else {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Manage nonprofit client records.</p>
        </div>
            ...
            <Card key={c.id} className="flex flex-col">
              ...
              <CardContent className="flex-1 space-y-3 text-sm">
                ...
                <div>
                  ...
                  <p className="text-foreground line-clamp-3">{c.mission}</p>
                </div>
                <div>
                  <p className="text-xs font-medium text-muted-foreground mb-1">Project Summary</p>
                  <p className="text-foreground line-clamp-3">{c.project_summary}</p>
                </div>
              </CardContent>
              <CardFooter className="pt-3 border-t flex justify-between items-center">
                <span className="text-xs text-muted-foreground">
                  Updated {format(new Date(c.updated_at), "MMM d, yyyy")}
                </span>
      ...
      <Dialog open={dialogOpen} onOpenChange={(open) => !open && closeDialog()}>
        <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              {editingClient
                ? "Update the client record below."
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="space-y-1.5">
              ...
              <Textarea
                ...
                maxLength={5000}
                aria-invalid={!!errors.project_summary}
              />
