...
// the edge function with their bearer JWT. An email prefetch (SafeLinks/AV) that
// merely opens this URL carries no session and triggers nothing.
type Status =
...
export default function ConfirmAdminPage() {
  ...
  // T-G: require the invited user to be signed in before anything can happen.
  if (!user) {
    ...
    return (
      <Shell>
        ...
        <p className="text-muted-foreground text-sm">
          For your security, confirm your admin role while signed in to the account it was issued
          to.
        </p>
        <Button asChild>
  ...
  const onConfirm = async () => {
    ...
    try {
      ...
      // Best-effort: provision a Freescout help-desk account (no-ops if not ready).
      try {
        await invokeEdge("freescout-provision-admin", {
    ...
    return (
      <Shell>
        ...
        <h2 className="text-xl font-bold text-foreground">
          {status === "already"
            ? "Your admin role was already confirmed"
            : "Your admin role is active"}
        </h2>
        <p className="text-muted-foreground text-sm">
  ...
  if (status === "expired" || status === "not_owner" || status === "error") {
    ...
    const body =
      status === "expired"
        ? "Ask an administrator to send a fresh admin invitation."
        : status === "not_owner"
  ...
  return (
    <Shell>
      <ShieldCheck className="h-10 w-10 text-primary mx-auto" />
      <h2 className="text-xl font-bold text-foreground">Confirm your admin role</h2>
      <p className="text-muted-foreground text-sm">
        You're signed in as <span className="font-medium text-foreground">{user.email}</span>. Click
        below to activate your admin privileges in the Tech Fleet Network.
      </p>
      <Button className="w-full" onClick={onConfirm} disabled={status === "submitting"}>
        {status === "submitting" ? "Confirming…" : "Confirm admin role"}
      </Button>
    </Shell>
  );
}
