...
export default function ConfirmTeacherPage() {
  ...
  useEffect(() => {
    ...
    const url = `https://${projectId}.supabase.co/functions/v1/confirm-teacher-role?token=${encodeURIComponent(token)}`;
    fetch(url)
      .then(async (res) => {
        ...
        if (res.ok) {
          if (data.already_confirmed) {
            setStatus("already");
            setMessage("Your teacher role was already confirmed.");
          } else {
            setStatus("success");
            setMessage("Your teacher role has been activated!");
          }
        } else {
          setStatus("error");
        ...
        <CardContent className="pt-6 text-center space-y-4">
          {status === "loading" && (
            <>
              ...
              <p className="text-muted-foreground">Confirming your teacher role…</p>
            </>
          )}
