# ADR 0049 — Auto-grant the "Founding Members" Discord role (event-sourced, self-healing)
...
| A worker drains the queue | **No server worker.** Drain is a *client login hook* (`src/hooks/use-discord-role-retry.ts`) — a member who never logs back in never gets their role |
