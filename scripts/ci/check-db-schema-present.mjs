...
// let ~50 objects vanish undetected). Benign GROWTH never fails (ADR-0052: a floor, not a band — the old
...
const BASELINES = {
  table: 202, // ADR-0062 −2 (refactor_kpi_catalog, refactor_kpi_daily)
  extension: 7,
  type: 25,
  view: 17,
  constraint: 20,
  rls_enabled: 202, // ADR-0062 −2 (refactor_kpi_catalog, refactor_kpi_daily)
  function: 418, // ADR-0062 −7 (refactor-KPIs subsystem + get_stuck_pending_email_count, clear_email_lane_cooldown)
  index: 393,
  trigger: 197, // ADR-0062 −1 (trg_broadcast_refactor_kpi_change)
  policy: 495, // +2 discord_roles admin read/write (ADR-0053)
  column: 2040, // ADR-0062 −21 refactor_kpi cols; re-pinned to post-merge corpus
};
...
async function main() {
  ...
  //     fails (ADR-0052: a floor, not a band — the old symmetric ±tol band re-tripped on every
